...
module.exports = function(RED) {
  ...
  if (
    !(
      1 &
      parseInt(
        (fs.statSync(gpioCommand).mode & parseInt('777', 8)).toString(8)[0]
      )
    )
  ) {
    RED.log.error(
      RED._('rpi-gpio.errors.needtobeexecutable', { command: gpioCommand })
    )
  ...
  var pinsInUse = {}
  var pinTypes = {
    out: RED._('rpi-gpio.types.digout'),
    tri: RED._('rpi-gpio.types.input'),
    up: RED._('rpi-gpio.types.pullup'),
    down: RED._('rpi-gpio.types.pulldown'),
    pwm: RED._('rpi-gpio.types.pwmout')
  }
  ...
  function GPIOInNode(n) {
    ...
    if (!pinsInUse.hasOwnProperty(this.pin)) {
      ...
      if (
        pinsInUse[this.pin] !== this.intype ||
        pinsInUse[this.pin] === 'pwm'
      ) {
        node.warn(
          RED._('rpi-gpio.errors.alreadyset', {
            pin: this.pin,
  ...
  function PiMouseNode(n) {
    ...
    node.child.stdout.on('data', function(data) {
      data = Number(data)
      if (data === 1) {
        node.send({ topic: 'pi/mouse', button: data, payload: 1 })
      } else {
        node.send({ topic: 'pi/mouse', button: data, payload: 0 })
      }
    })
  ...
  RED.httpAdmin.get(
    '/rpi-gpio/:id',
    RED.auth.needsPermission('rpi-gpio.read'),
    function(req, res) {
      res.json(pitype)
    }
  )
