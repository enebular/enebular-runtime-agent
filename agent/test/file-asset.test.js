...
test.serial(
  ...
  async t => {
    ...
    for (let i = 0; i < destPathForTest.length; i++) {
      t.true(fs.existsSync(path.join(agent._assetManager.dataDir(),
              destPathForTest[i],
              assets[i].id)))
...
test.serial(
  ...
  async t => {
    ...
    const integrityForTest = [
      TEST_WRONG_INTEGRITY,
      TEST_CRYPT_FILE,
      TEST_OK
    ]
...
test.serial(
  ...
  async t => {
    ...
    for (let i = 0; i < cmdForTest.length; i++) {
      ...
      const content =  `#!/bin/bash\n [[ ! -f ${assets[i].id} ]] && touch pre-hook${i}`
    ...
    for (let i = 0; i < cmdForTest.length; i++) {
      const s = ret.reportedStates.state.assets.assets[assets[i].id]
      t.is(s.state, 'deployed')
      t.true(fs.existsSync(path.join(agent._assetManager.dataDir(), 'pre-hook' + i)))
    }
  ...
  async t => {
    ...
    for (let i = 0; i < cmdForTest.length; i++) {
      if (cmdForTest[i].stage === 'postDeploy') {
        ...
        t.true(fs.existsSync(path.join(agent._assetManager.dataDir(), assets[i].id)))
      }
      const s = ret.reportedStates.state.assets.assets[assets[i].id]
      t.is(s.state, 'deployFail')
